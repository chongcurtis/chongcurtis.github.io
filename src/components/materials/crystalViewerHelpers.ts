import { atomicNumberToHexColor } from "@/components/materials/atomColors";
import { atomicRadii } from "@/components/materials/atomSizes";
import {
    BufferGeometry,
    Line,
    LineBasicMaterial,
...
export const fracCoordToCartesianCoord = (fracCoord: number[], lattice: number[][]): number[] => {
    const [lattice0, lattice1, lattice2] = lattice;

    return [
        fracCoord[0] * lattice0[0] + fracCoord[1] * lattice1[0] + fracCoord[2] * lattice2[0],
        fracCoord[0] * lattice0[1] + fracCoord[1] * lattice1[1] + fracCoord[2] * lattice2[1],
        fracCoord[0] * lattice0[2] + fracCoord[1] * lattice1[2] + fracCoord[2] * lattice2[2],
    ];
};
...
export const getParallelepipedCornerCoords = (lattice: number[][]): number[][] => {
...
export const drawEdge = (scene: Scene, edge: Edge) => {
    ...
    const points = [new Vector3(p1[0], p1[1], p1[2]), new Vector3(p2[0], p2[1], p2[2])];
